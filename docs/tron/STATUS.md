...
**Follow-up (emergent-находка финального ревью F+G, 2026-07-12):** `forge test --fork-url …/jsonrpc --fork-block-number <исторический>` на tron сейчас проходит МОЛЧА: block env пинится на указанный номер, но state (из-за tip-only ограничения выше) читается на текущем tip — смесь. Спека же обещает «на исторические блоки — явная ошибка». Решить в следующем заходе: явный bail/warn при `fork_block_number` на tron-пути forge test ЛИБО правка формулировки спеки на «tip-only, block env настраиваем». Рекомендация ревью — warn или bail.
